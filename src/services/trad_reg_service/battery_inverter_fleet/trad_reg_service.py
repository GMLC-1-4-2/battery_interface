...
class TradRegService():
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fleet =  BatteryInverterFleet(GridInfo = self.grid) #establish the battery inverter fleet with a grid
    ...
    def request_loop(self, filename='08 2017.csv'): ### (TODO) add keywords for date and start hour, duration. add serv_type 'trad_reg'(default), 'dynm_reg'
        ...
        for row in df_RegA.iterrows():
            ...
            if start_time <= ts < end_time:
                ...
                p = row[1][1]*(self.fleet.max_power_discharge*self.fleet.num_of_devices)*0.2  ### need to multiple a "AReg" value from the fleet - "Assigned Regulation (MW)"
