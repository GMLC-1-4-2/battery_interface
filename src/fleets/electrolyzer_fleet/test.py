...
def fleet_test1(fleet):
    ...
    y1.plot(ts, P_togrid, label='P_togrid')
    ...
    grid()
    show()
    fig1.savefig(join(base_path, "Ey_result_ALL_P_%s.png" % str(datetime.utcnow().strftime('%d_%b_%Y_%H_%M_%S'))),
                 bbox_inches='tight')
    ...
    y1.axhline(y=0, color='k', linestyle='-', alpha=0.5)
    ...
    grid()
    show()
    fig1.savefig(join(base_path, "Ey_result_All_Ps_%s.png" % str(datetime.utcnow().strftime('%d_%b_%Y_%H_%M_%S'))),
                 bbox_inches='tight')
...
def fleet_test2(fleet):
    ...
    # Plot the results
    fig1, y1 = subplots(figsize=(20, 12))
    #p1, = y1.plot(P_service, label='service power', color='b')
    p2, = y1.plot(P_togrid, label='power to grid', color='k')
    y2 = y1.twinx()
    p3, = y2.plot(f, label='frequency', color='r')
    y2.set_ylabel('f (Hz)')
    plots = [p2, p3]
    y1.set_ylabel('Power (kW)')
    ...
    y2.scatter(f, array(P_togrid)/130, label='P_togrid')
    y2.set_ylabel('Power Consumption(p.u.)')
    y2.set_xlabel('Hz')
    grid()
    show()
    fig2.savefig(join(base_path, "Ey_result_P_service_%s.png" % str(datetime.utcnow().strftime('%d_%b_%Y_%H_%M_%S'))),
                 bbox_inches='tight')
